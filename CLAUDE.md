...
- **`src/matchstick_man/app.py`** — defines `class App`. `__init__` calls `pyxel.init(WIDTH, HEIGHT, title=TITLE)`, loads `matchstick.pyxres`, and initializes a `Player` centered on screen. `run()` enters the blocking `pyxel.run(self.update, self.draw)` loop. The init/run split (vs. Pyxel's bundled examples, which call `pyxel.run` from inside `__init__`) is deliberate — it lets tests construct `App` without blocking. Sprite u-coordinates per `PlayerState` live in `_SPRITE_U` at the top of the module. `WIDTH`, `HEIGHT`, and `TITLE` are inlined; promote to a `constants.py` only when there are >10 game constants.
